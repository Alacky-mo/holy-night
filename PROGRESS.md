...
| 阶段    | 状态    | 提交        | 主要内容                                                        |
| ----- | ----- | --------- | ----------------------------------------------------------- |
| 0 初始化 | ✅ 已完成 | `55d745f` | Astro4 + Tailwind v3 + Vue 岛屿 + Notion SDK 链路、.env 配置、空数据降级 |
| 1 页面  | ✅ 已完成 | `c5885cc` | 首页 / 归档 / 分类 / 标签 / 详情 5 页 + Notion 块渲染、分页、上下篇数据基础          |
| 2 视觉  | ✅ 已完成 | `0b0a00b` | 双主题（深色默认 / 浅色）、魔法使之夜配色、Notion 全块样式、主题切换                     |
| 3 交互  | ✅ 已完成 | `e2e2857` | 目录跟随、返回顶部、上一篇 / 下一篇、响应式、入场动效、404、空状态、favicon                |
| 4 部署  | 🔶 代码就绪 · 待用户部署 | `ef96f3b` | SEO（OG/Twitter/sitemap/robots）、图片本地化、DEPLOY.md；Git 远程 / Vercel / Deploy Hook 需用户操作（见 `DEPLOY.md`） |
| 5 重设计 | ✅ 已完成 | `16ca337` | 「双世界」视觉重设计：月白+琥珀 token 迁移、固定背景三层、Hero/丝带/引言带、暖调文章面板、菱形归档时间轴、导航滚动毛玻璃、Cormorant 西文字体、背景图入库 `public/backgrounds/` |

> 注：2026-09-22 对全部提交信息做了精简并 force-push 重写历史，提交号已全部变更（上表为新号），文件内容无变化。
