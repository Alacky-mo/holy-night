...
| 2026-09-23 | 画框陪读组件修订 | 实体玻璃框改为四边 mask 透明羽化（移除边框/玻璃/暗角/底边渐变/文字标签，双主题自动适配）；尺寸缩小（桌面 176 / ≤1024 140 / ≤380 120）；× 永久关闭改为有珠头像开合 toggle，状态键改为 alice-companion-collapsed（一次性迁移旧 dismissed 标记）；挂载从 BaseLayout 移至 PostLayout，仅文章页显示 | 实体框视觉与「双世界」仍有割裂且遮挡正文；羽化加缩小显著降低遮挡感；仅文章页有陪读语境、桌面右侧有空隙 | 放弃全局显示（列表页遮挡卡片）与永久关闭（无唤回入口）；头像 toggle 保留一键收起与恢复 |
